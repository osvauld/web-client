(function () {
	...
	const improvedLoginDetection = () => {
	    ...
	    function isUsernameField(input) {
	        ...
	        return inputProps.some((prop) => usernameKeywords.some((keyword) => prop.includes(keyword)));
	    }
	    function hasLoginIndicators(container) {
	        const buttonText = Array.from(container.querySelectorAll('button, input[type="submit"], input[type="button"]'))
	            .map((el) => (el instanceof HTMLInputElement ? el.value : el.textContent) || "")
	            .join(" ")
	            .toLowerCase();
	        const hasLoginButton = loginKeywords.some((keyword) => buttonText.includes(keyword));
	        ...
	        const hasLoginAttr = containerAttrs.some((attr) => loginAttrKeywords.some((keyword) => attr.includes(keyword)));
	        return hasLoginButton || hasLoginAttr;
	    }
	    function getCredentialFields(container) {
	        ...
	        if (!usernameInput) {
	            usernameInput = inputs.find(isUsernameField);
	        }
	        return { usernameInput, passwordInput };
	    }
	    async function credentialCaptureHandler(username, password) {
	    ...
	    function attachLoginListener(container) {
	        const { usernameInput, passwordInput } = getCredentialFields(container);
	        if (usernameInput && passwordInput) {
	            const handler = () => credentialCaptureHandler(usernameInput.value, passwordInput.value);
	            ...
	            const loginButtons = Array.from(container.querySelectorAll('button, input[type="submit"], input[type="button"]')).filter((button) => {
	                const buttonText = (button instanceof HTMLInputElement
	                    ? button.value
	                    : button.textContent) || "";
	                return loginKeywords.some((keyword) => buttonText.toLowerCase().includes(keyword));
	            });
	    ...
	    function detectLoginForms() {
	        const potentialContainers = [
	            ...
	            ...document.querySelectorAll("div[class*='login'], div[class*='signin'], div[id*='form']"),
	        ];
	    ...
	    function initializeMutationObserver() {
	                    ...
	                    mutation.addedNodes.forEach((node) => {
	                        if (node instanceof HTMLElement) {
	                            if (node instanceof HTMLFormElement || hasLoginIndicators(node)) {
	                                console.log("Dynamically added login container detected:", node);
	                                attachLoginListener(node);
	                            }
	                            else {
	                                const containers = node.querySelectorAll("form, div[class*='login'], div[class*='signin'], div[id*='form']");
	                                containers.forEach((container) => {
	                                    if (container instanceof HTMLElement &&
	                                        (container instanceof HTMLFormElement ||
	                                            hasLoginIndicators(container))) {
	                                        console.log("Dynamically added login container detected:", container);
	                                        attachLoginListener(container);
	                                    }
	                                });
	                            }
	        ...
	        if (document.body) {
	            ...
	            console.warn("Document body not available, MutationObserver not initialized");
	        }
	    }
	...
	const postLoginContent = (data) => {
	    ...
	    iframe.onload = () => {
	        iframe.contentWindow?.postMessage(data, `${browser.runtime.getURL("suggestions.html")}`);
	    };
	    window.addEventListener("message", (event) => {
	        ...
	        if (save) {
	            browser.runtime
	                ...
	                .then((response) => {
	                iframe.contentWindow?.postMessage({ confirmation: true, id: "osvauld", ...response }, `${browser.runtime.getURL("suggestions.html")}`);
	            })
	                .catch((error) => {
