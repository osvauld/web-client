import {
	...
	FetchGroupsWithoutAccessResponse,
	fetchUsersByGroupIdsResponse,
...
export const fetchAllUserGroups =
	async (): Promise<FetchAllUserGroupsResponse> => {
...
export const fetchGroupUsers = async (
	id: string,
): Promise<FetchGroupUsersResponse> => {
	const { token, baseUrl } = await getTokenAndBaseUrl();
...
export const fetchUsersByGroupIds = async (
	ids: string[],
): Promise<fetchUsersByGroupIdsResponse> => {
	const headers = new Headers();
	const { token, baseUrl } = await getTokenAndBaseUrl();
	headers.append("Authorization", `Bearer ${token}`);
	headers.append("Content-Type", "application/json");
...
export const shareCredentialsWithGroups = async (
	payload: ShareCredentialsWithGroupsPayload,
) => {
	const headers = new Headers();
	const signatureResponse = await sendMessage("hashAndSign", {
		message: JSON.stringify(payload),
...
export const fetchGroupsWithoutAccess = async (
	folderId: string,
): Promise<FetchGroupsWithoutAccessResponse> => {
	const { token, baseUrl } = await getTokenAndBaseUrl();
...
export const fetchCredentialFieldsByGroupId = async (
	groupId: string,
): Promise<FetchGroupCredentialResponse> => {
	const { token, baseUrl } = await getTokenAndBaseUrl();
...
export const fetchUsersWithoutGroupAccess = async (
	groupId: string,
): Promise<FetchUsersWithoutGroupAccess> => {
	const { token, baseUrl } = await getTokenAndBaseUrl();
...
export const fetchCredentialGroups = async (
	credentialId: string,
): Promise<FetchCredentialGroupsResponse> => {
	const headers = new Headers();
	const { token, baseUrl } = await getTokenAndBaseUrl();
	headers.append("Authorization", `Bearer ${token}`);
...
export const removeGroupFromCredential = async (
	...
	return await fetch(
		`${baseUrl}/credential/${credentialId}/remove-group-access`,
		{
			method: "POST",
			headers,
			body: JSON.stringify({ groupIds: [groupId] }),
